...
class UserProfileView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_authenticated:
            postlist = WeightEntry.objects.all().filter(
                user=self.kwargs["pk"]).order_by("-recorded", "-updated", "-created")
        else:
            postlist = WeightEntry.objects.all().order_by(
...
class UserWeightEntryListView(ListView):
    """user's profile view"""
    model = WeightEntry
    ...
    def get_context_data(self, **kwargs):
        context = super(UserWeightEntryListView,
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            queryset = queryset.filter(user=self.kwargs["pk"]).order_by(
                "-recorded", "-updated", "-created")
        else:
